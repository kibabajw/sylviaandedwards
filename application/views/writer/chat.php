...
<script>
    $(document).ready(function(){
        ...
        chat.fetch_contacts = function(){
            var cont_url = '<?php echo base_url() ?>index.php/writer/Writer_Chat_Controller/contacts';
            $.get(cont_url, function(response){
                $(".contacts_list").html(response);
            });
        }
        chat.fetch_my_msg = function(){
            from_who = $("#chat_recipient").html();
            $.ajax({
            url: '<?php echo base_url() ?>index.php/writer/Writer_Chat_Controller/get_my_msg',
            type: 'post',
            ...
            // var cont_url = '<?php echo base_url() ?>index.php/writer/Writer_Chat_Controller/get_my_msg';
            // $.get(cont_url, function(response){
            //     $(".base_sent").html(response);
            // });
        }
        chat.send_msg = function(){
            var cont_url = '<?php echo base_url() ?>index.php/writer/Writer_Chat_Controller/get_other_msg';
            $.get(cont_url, function(response){
        ...
        chat.fetch_my_msg();
        // chat.interval = setInterval(chat.send_msg, 3000);
        // chat.send_msg();
...
chat.throwMessage = function(recipient, message){
    if ($.trim(recipient).length != 0 && $.trim(message).length != 0) {
        ...
        $.ajax({
        url: '<?php echo base_url() ?>index.php/writer/Writer_Chat_Controller/insert_msg',
        type: 'post',
